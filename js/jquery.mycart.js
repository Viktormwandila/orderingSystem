    ...
    if (!$("#" + idCartModal).length) {
      $("body").append(
        '<div class="modal fade" id="' +
          ...
          '<button type="button" id="order" name="order" class="btn btn-default" data-dismiss="">Order Now</button>' +
          "</div>" +
          "</div>" +
          "</div>" +
          "</div>"
      ...
      $(window).scroll(function() {
        if ($(window).scrollTop() >= cartIconBottom) {
          $cartIcon
            .css("position", "fixed")
            .css("z-index", "999")
            .addClass(classAffixMyCartIcon);
        } else {
          $cartIcon
            .css("position", cartIconPosition)
            .css("background-color", "inherit")
            .removeClass(classAffixMyCartIcon);
        }
      });
    }
